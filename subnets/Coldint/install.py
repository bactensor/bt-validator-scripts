...
def main():
    ...
    sec = find_secret("coldint_vali", 29, 29000, 'dev29000')
    
    local_env_file = "subnets/Coldint/.env"
    ...
    if 'database_password' in sec:
        database_password = sec['database_password']    
    
    
    local_json_file = 'subnets/Coldint/ecosystem.config.js'
...
module.exports = {{
    apps: [
        ...
        {{
            ...
            script: 'python3',
            interpreter: 'none',
            args: 'neurons/verifier/app.py --subtensor.network ws://REDACTED:9944 --neuron.model_endpoint "http://127.0.0.1:5005" --logging.debug --logging.trace --database.host 127.0.0.1 --database.port 6379 --database.password {database_password} --netuid 29 --axon.port 10704 --axon.external_port 10704 --wallet.name {wallet_name} --wallet.hotkey {hotkey_name}'
        }}
    ]
}};
    """
    ...
    print(f"Data has been written to {local_json_file}")
    ...
    connection.put(local_json_file, remote='coldint_vali/ecosystem.config.js')
    connection.put(local_env_file, remote='coldint_vali/.env')
    connection.put('subnets/Coldint/install.sh', remote='coldint_vali/install.sh')
    connection.put('subnets/Coldint/run_vali.sh', remote='coldint_vali/run_vali.sh')
