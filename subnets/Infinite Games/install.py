...
def main():
    ...
    remote_directory = '~/infinite_games'
    print(REMOTE_HOTKEY_DIR)
    ...
    time.sleep(3)
    sec = find_secret("infinite-games", 6, 155, 'dev155')    
        
    # Create .env file locally
    local_env_file = "subnets/Infinite Games/.env"
    ...
    connection.put(local_env_file, remote='infinite_games/.env')
    connection.put('subnets/Infinite Games/install.sh', remote='infinite_games/install.sh')
    connection.put('subnets/Infinite Games/run_vali.sh', remote='infinite_games/run_vali.sh')
