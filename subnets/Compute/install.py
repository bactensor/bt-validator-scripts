...
def main():
    ...
    time.sleep(3)
    sec = find_secret("compute", 27, 15, 'dev15')
    ...
    local_env_file = "subnets/Compute/.env"
    with open(local_env_file, 'w') as env_file:
        ...
        env_file.write(f"DEALLOCATION_NOTIFY_URL={DEALLOCATION_NOTIFY_URL}\n")
        env_file.write(f"STATUS_NOTIFY_URL={STATUS_NOTIFY_URL}\n")
    ...
    connection.put(local_env_file, remote='compute/.env')
    connection.put('subnets/Compute/install.sh', remote='compute/install.sh')
    connection.put('subnets/Compute/run_vali.sh', remote='compute/run_vali.sh')
...
# python3 install.py shane@37.27.180.98 /Users/mac/Downloads/project/wallet/wombo/hotkeys/JJa 
