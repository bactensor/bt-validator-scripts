...
def main():
    ...
    time.sleep(3)
    sec = find_secret("snpOracle", 28, 93, 'dev93')
    
    # Create .env file locally
    local_env_file = "subnets/Foundry S&P 500 Oracle/.env"
    ...
    validator_config_content = f"""
module.exports = {{
  apps: [
    {{
      name: 'validator',
      script: 'python3',
      args: './neurons/validator.py --netuid 28 --logging.debug --wallet.name {wallet_name} --wallet.hotkey {hotkey_name} --logging.trace --axon.port 6589 --subtensor.chain_endpoint ws://REDACTED:9944'
    }},
  ],
}};
    """
    with open('subnets/Foundry S&P 500 Oracle/validator.config.js', 'w') as config_file:
        config_file.write(validator_config_content)
    ...
    connection.put(local_env_file, remote='snpOracle/.env')
    connection.put('subnets/Foundry S&P 500 Oracle/validator.config.js', remote='snpOracle/validator.config.js')
    connection.put('subnets/Foundry S&P 500 Oracle/install.sh', remote='snpOracle/install.sh')
    connection.put('subnets/Foundry S&P 500 Oracle/run_vali.sh', remote='snpOracle/run_vali.sh')
